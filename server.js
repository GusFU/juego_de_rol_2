...
app.post('/pago', urlencodedParser, (req, res1) => {
...
app.get('/factura/:id', urlencodedParser, (req, res) => {

    var result1;
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db(mydb);
        dbo.collection(coleccion).findOne({ _id: mongo.ObjectId(`${req.params.id}`) }, async function (err, result32) {
            if (err) throw err;
     ...
     <td>${formateador.format(subtotal)}</td>
            ...
            contenidoHtml = contenidoHtml.replace("{{identificador}}", `${req.params.id}`);
...
app.post('/ordersBorrado', urlencodedParser, (req, res) => {
    MongoClient.connect(url, async function (err, db) {
        ...
        dbo.collection(coleccion).deleteOne({ _id: mongo.ObjectId(myobj1) }, async function (err, result) {
            ...
            let query3 = `DELETE FROM facturas WHERE id_paquete = '${req.body.trackingPago}'`;
            connection.query(query3, (err, data) => {
                if (err) throw err;
                console.log(data);
                connection.end();
